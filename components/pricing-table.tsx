...
} from "@/hooks/use-create-subscription";
...
export function PricingTable({
  ...
  const handleOpenCheckout = async (
    ...
    const order = await createSubscription({ option });
        ...
        >
          {config.products.map((product) => {
            ...
            if (!plan.custom && !primaryLineItem) {
              throw new Error(
                `Primary line item not found for plan ${plan.id}`
              );
            }
            ...
            return (
              <PricingItem
                ...
                onCheckout={(pro) =>
                  handleOpenCheckout(pro, plan.id, product.type, product.name)
                }
                type={product.type}
      ...
      <div className={"flex flex-col space-y-6"}>
        ...
        <div className={"flex flex-col space-y-2"}>
          ...
          <If condition={props.type == "custom"}>
            <div className="font-heading flex items-center text-3xl font-semibold tracking-tighter">
              Custom Pricing
            </div>
          </If>
          ...
          <If condition={props.plan.name}>
            ...
            >
              <span>
                <If condition={props.plan.interval} fallback={"Lifetime"}>
                  {(interval) =>
              ...
              <If condition={lineItem && lineItem?.type !== "flat"}>
                ...
                >
                  ...
                  <If condition={lineItem?.unit}>Per {lineItem?.unit} usage</If>
                </span>
              </If>
            </span>
          </If>
                    ...
                    items: [
                      {
                        ...
                        price: {
                          ...
                          unit_price: {
                            amount: props.plan.lineItems[0]?.cost
                              ? String(convert(props.plan.lineItems[0]?.cost))
                              : "",
...
function DefaultCheckoutButton(
  ...
  return (
    ...
    >
      <span>
        <p>{label.replace("{{plan}}", t(name, { defaultValue: name }))}</p>
      </span>
...
const LineItemPrice = ({
  ...
  }, [alwaysDisplayMonthlyPrice, currency, isYearlyPricing, lineItem]);
